...
const NON_FOOD_KEYWORDS = [ 'resort', 'inn', 'stay', 'cottage', 'residency', 'lodge', 'dorm', 'hostel', 'room', 'living', 'apartment', 'villa', 'bnb', 'homestay', 'temple', 'shrine', 'worship', 'church', 'mosque', 'fort', 'park', 'garden', 'museum', 'dam', 'falls', 'view point', 'market', 'stand', 'store', 'shop', 'complex', 'race', 'bridge', 'river', 'lake' ];
const ACCOMMODATION_KEYWORDS = [ 'resort', 'inn', 'stay', 'cottage', 'residency', 'lodge', 'dorm', 'hostel', 'room', 'living', 'apartment', 'villa', 'bnb', 'homestay', 'hotel', 'palace' ];
...
export default function Home() {
  ...
  // --- SETTINGS STATE ---
  const [userSettings, setUserSettings] = useState({ name: 'Traveler', email: '', currency: 'INR', language: 'English', notifications: true, theme: 'light' });
  ...
  useEffect(() => { if(session?.user?.email) { setUserSettings(prev => ({ ...prev, email: session.user.email })); } }, [session]);
  ...
  const mapCenter = useMemo(() => { if (tripPlan.length > 0) return { lat: tripPlan[0].lat, lng: tripPlan[0].lng }; return { lat: 12.9716, lng: 77.5946 }; }, [tripPlan]);

  const handleViewChange = (view: NavView) => { setActiveView(view); if (view === 'DASHBOARD') { setIsWizardActive(false); setShowCreateModal(false); } };

  // --- HANDLERS ---
  const openAiAssistant = (place: Place) => { setAiActivePlace(place); setAiChatHistory([{ id: 'system', user: 'Genius', text: `Hello! I'm your expert guide for ${place.name}. Ask me about tickets, dress code, or best times to visit!`, time: 'Now', isMe: false }]); setAiModalOpen(true); };
  const handleAiAsk = (query: string = aiQuery) => { if(!query.trim()) return; const userMsg: ChatMessage = { id: Date.now().toString(), user: 'You', text: query, time: 'Now', isMe: true }; setAiChatHistory(prev => [...prev, userMsg]); setAiQuery(''); setTimeout(() => { const responses = [`Great question about ${aiActivePlace?.name}! The best time to visit is early morning.`, `Wear comfortable shoes!`, `Entry is typically free.`, `It's fantastic for photography!`]; const randomResponse = responses[Math.floor(Math.random() * responses.length)]; setAiChatHistory(prev => [...prev, { id: (Date.now()+1).toString(), user: 'Genius', text: randomResponse, time: 'Now', isMe: false }]); }, 1500); };
  const calculateRoute = async () => { if (tripPlan.length < 2) { alert("Add at least 2 places to see a route."); return; } setIsRouting(true); const directionsService = new google.maps.DirectionsService(); const origin = { lat: tripPlan[0].lat, lng: tripPlan[0].lng }; const destination = { lat: tripPlan[tripPlan.length - 1].lat, lng: tripPlan[tripPlan.length - 1].lng }; const waypoints = tripPlan.slice(1, -1).map(p => ({ location: { lat: p.lat, lng: p.lng }, stopover: true })); directionsService.route({ origin, destination, waypoints, travelMode: google.maps.TravelMode.DRIVING }, (result, status) => { if (status === 'OK' && result) { setDirectionsResponse(result); } else { alert("Could not calculate route. Try fewer stops."); } setIsRouting(false); }); };
  const handleCopyLink = () => { navigator.clipboard.writeText(inviteLink); alert("Invite link copied to clipboard! 📋"); };
  ...
  // PACKING HANDLERS
  const addPackingItem = () => { if(!newPackingItem.trim()) return; setPackingList([...packingList, { id: Date.now().toString(), text: newPackingItem, checked: false }]); setNewPackingItem(''); };
  const togglePackingItem = (id: string) => { setPackingList(packingList.map(item => item.id === id ? { ...item, checked: !item.checked } : item)); };

  const totalCost = expenses.reduce((a,b) => a + b.amount, 0); const costPerPerson = tripMembers.length > 0 ? totalCost / tripMembers.length : 0; const myTotalPaid = expenses.filter(e => e.who === 'You').reduce((a,b) => a + b.amount, 0); const myBalance = myTotalPaid - costPerPerson;
  ...
  const generateOptionsForStep = (stepIdx: number, allPlaces: Place[], currentTrip: Place[], stepsList = dynamicSteps, initialCoords: {lat: number, lng: number} | null = startCoords) => {
    ...
    const isAccommodation = (p: Place) => { const t = (p.type + " " + p.name + " " + p.description).toLowerCase(); return ACCOMMODATION_KEYWORDS.some(kw => t.includes(kw)); };
    ...
    if (isMeal) { const foodCandidates = candidates.filter(p => !isNonFoodPlace(p)); if (foodCandidates.length > 0) candidates = foodCandidates; } else { const activityCandidates = candidates.filter(p => !isAccommodation(p)); if (activityCandidates.length > 0) candidates = activityCandidates; }
    ...
    if (candidates.length < 4) { const remainingNeeded = 4 - candidates.length; let fallbackCandidates = allPlaces.filter(p => !currentTrip.some(picked => picked.id === p.id) && !candidates.some(c => c.id === p.id)); if (isMeal) { const betterFoodFallback = fallbackCandidates.filter(p => (p.name + p.type).toLowerCase().match(/restaurant|mess|bhavan|kitchen|canteen|cafe|dining/) && !isNonFoodPlace(p)); if (betterFoodFallback.length > 0) fallbackCandidates = betterFoodFallback; else fallbackCandidates = fallbackCandidates.filter(p => !isNonFoodPlace(p)); } else { fallbackCandidates = fallbackCandidates.filter(p => !isAccommodation(p)); } if (referencePoint) { fallbackCandidates.sort((a, b) => { const distA = Math.sqrt(Math.pow(a.lat - referencePoint!.lat, 2) + Math.pow(a.lng - referencePoint!.lng, 2)); const distB = Math.sqrt(Math.pow(b.lat - referencePoint!.lat, 2) + Math.pow(b.lng - referencePoint!.lng, 2)); return distA - distB; }); } candidates = [...candidates, ...fallbackCandidates.slice(0, remainingNeeded)]; }
              ...
              <div className="bg-white p-6 rounded-3xl shadow-2xl w-full max-w-lg relative overflow-y-auto max-h-[90vh]">
                 ...
                 <div className="mb-4 relative">
                    ...
                    {showSuggestions && (
                       <div className="absolute top-full w-full bg-white border border-gray-100 rounded-xl shadow-xl z-50 mt-1 max-h-40 overflow-y-auto">
                          {citySuggestions.map((s,i)=><div key={i} onClick={()=>selectSuggestion(s)} className="p-3 hover:bg-blue-50 cursor-pointer text-sm font-bold border-b border-gray-50">📍 {s}</div>)}
                       </div>
                    )}
                 </div>
                 ...
                 <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                       {TRIP_VIBES.map((v) => (
                          <button key={v.id} onClick={() => toggleVibe(v.id)} className={`px-3 py-2 rounded-lg text-xs font-bold border transition-all ${selectedVibes.includes(v.id) ? 'bg-black text-white border-black shadow-md' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-400'}`}>{v.label}</button>
                       ))}
                    </div>
                 </div>
                 ...
                 <div className="mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                       {[{ id: 'SOLO', label: '🧍 Solo' }, { id: 'FRIENDS', label: '👯 Friends' }, { id: 'FAMILY', label: '👨‍👩‍👧‍👦 Family' }, { id: 'CORPORATE', label: '💼 Corporate' }].map((t) => (
                          <button key={t.id} onClick={() => setGroupType(t.id)} className={`p-3 rounded-xl border text-left transition-all ${groupType === t.id ? 'bg-blue-50 border-blue-500' : 'border-gray-200'}`}><div className="font-bold text-xs">{t.label}</div></button>
                       ))}
                    </div>
                 </div>
