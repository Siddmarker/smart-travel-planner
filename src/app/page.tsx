...
const PATH_OPTIONS = { strokeColor: '#2563EB', strokeOpacity: 0.5, strokeWeight: 4 }; // Lighter generic path
...
export default function Home() {
  ...
  const handleAiAsk = (query: string = aiQuery) => {
    ...
    // Simulate AI Response (In production, call your AI API here)
    setTimeout(() => {
      const responses = [
  ...
  // --- COLLAB LOGIC ---
  const addMember = () => { if(newMemberName.trim() && !tripMembers.includes(newMemberName.trim())) { setTripMembers([...tripMembers, newMemberName.trim()]); setNewMemberName(''); } };
  ...
  const generateOptionsForStep = (stepIdx: number, allPlaces: Place[], currentTrip: Place[], stepsList = dynamicSteps, initialCoords: {lat: number, lng: number} | null = startCoords) => {
    setIsLoadingOptions(true); const stepConfig = stepsList[stepIdx];
    let referencePoint = null; if (currentTrip.length > 0) { const last = currentTrip[currentTrip.length - 1]; referencePoint = { lat: last.lat, lng: last.lng }; } else if (initialCoords && stepIdx === 0) { referencePoint = initialCoords; }
    const isNonFoodPlace = (p: Place) => { const t = (p.type + " " + p.name + " " + p.description).toLowerCase(); return NON_FOOD_KEYWORDS.some(kw => t.includes(kw)); };
    let candidates = allPlaces.filter(p => { const searchStr = `${p.type} ${p.description} ${p.name} ${p.vibes?.join(' ')}`.toLowerCase(); let matchesType = stepConfig.types.some((t: string) => searchStr.includes(t)); if (selectedVibes.length > 0) { const vibeKeywords = selectedVibes.flatMap(vid => TRIP_VIBES.find(v => v.id === vid)?.keywords || []); if (vibeKeywords.some(k => searchStr.includes(k))) matchesType = true; } const alreadyPicked = currentTrip.some(picked => picked.id === p.id); return matchesType && !alreadyPicked; });
    const isMeal = stepConfig.id.includes('DINNER') || stepConfig.id.includes('LUNCH'); if (isMeal) { const foodCandidates = candidates.filter(p => !isNonFoodPlace(p)); if (foodCandidates.length > 0) candidates = foodCandidates; }
    if (diet !== 'ANY') { candidates = candidates.filter(p => { const text = (p.name + " " + p.description).toLowerCase(); const isNonVeg = NON_VEG_KEYWORDS.some(kw => text.includes(kw)); if (diet === 'VEG' || diet === 'VEGAN' || diet === 'JAIN') return !isNonVeg || text.includes('pure veg'); return true; }); }
    candidates = candidates.filter(p => { const price = p.price_tier || (p.price_level === 0 ? 'Budget' : p.price_level === 4 ? 'Luxury' : 'Standard'); if (budget === 'LOW') return price === 'Budget' || (p.price_level !== undefined && p.price_level <= 1); if (budget === 'HIGH') return price === 'Luxury' || price === 'Premium' || (p.price_level !== undefined && p.price_level >= 3); return true; });
    if (candidates.length < 4) { const remainingNeeded = 4 - candidates.length; let fallbackCandidates = allPlaces.filter(p => !currentTrip.some(picked => picked.id === p.id) && !candidates.some(c => c.id === p.id)); if (isMeal) { const betterFoodFallback = fallbackCandidates.filter(p => (p.name + p.type).toLowerCase().match(/restaurant|mess|bhavan|kitchen|canteen|cafe|dining/) && !isNonFoodPlace(p)); if (betterFoodFallback.length > 0) fallbackCandidates = betterFoodFallback; else fallbackCandidates = fallbackCandidates.filter(p => !isNonFoodPlace(p)); } else { fallbackCandidates = fallbackCandidates.filter(p => !p.type.includes('lodging') && !p.name.toLowerCase().includes('resort')); } if (referencePoint) { fallbackCandidates.sort((a, b) => { const distA = Math.sqrt(Math.pow(a.lat - referencePoint!.lat, 2) + Math.pow(a.lng - referencePoint!.lng, 2)); const distB = Math.sqrt(Math.pow(b.lat - referencePoint!.lat, 2) + Math.pow(b.lng - referencePoint!.lng, 2)); return distA - distB; }); } candidates = [...candidates, ...fallbackCandidates.slice(0, remainingNeeded)]; }
    candidates = candidates.map(p => { let score = calculateRelevanceScore(p, groupType); let distanceText = ""; if (referencePoint) { const distDeg = Math.sqrt(Math.pow(p.lat - referencePoint.lat, 2) + Math.pow(p.lng - referencePoint.lng, 2)); const distKm = distDeg * 111; distanceText = `${distKm.toFixed(1)} km away`; if (distKm < 3) score += 30; else if (distKm < 8) score += 15; else if (distKm > 20) score -= 20; } const text = (p.name + " " + p.description + " " + (p.vibes?.join(' ') || '')).toLowerCase(); selectedVibes.forEach(vid => { const keywords = TRIP_VIBES.find(v => v.id === vid)?.keywords || []; if (keywords.some(k => text.includes(k))) score += 25; }); return { ...p, aiScore: score, distanceFromLast: distanceText }; });
    candidates.sort((a, b) => (b.aiScore || 0) - (a.aiScore || 0)); setStepOptions(candidates.slice(0, 4)); setIsLoadingOptions(false);
  };

  const handleOptionSelect = (place: Place) => { const newTrip = [...tripPlan, place]; setTripPlan(newTrip); const nextStep = currentStepIndex + 1; if (nextStep < dynamicSteps.length) { setCurrentStepIndex(nextStep); generateOptionsForStep(nextStep, allCityPlaces, newTrip, dynamicSteps, startCoords); } else { setIsWizardActive(false); setActiveView('PLAN'); } };
  const calculateRelevanceScore = (place: Place, group: string) => { let score = 50; const safety = place.safety_score || 5; const trend = place.trend_score || 5; if (group === 'FRIENDS') { score += trend * 5; } if (group === 'FAMILY') { score += safety * 5; } if (diet !== 'ANY' && place.description?.toLowerCase().includes(diet.toLowerCase())) score += 30; return score; };
  const handleLogout = async () => { await supabase.auth.signOut(); window.location.reload(); };
  const removeFromTrip = (id: string) => setTripPlan(tripPlan.filter(p => p.id !== id));
  const calculateDays = () => { if(!dates.start || !dates.end) return 1; return Math.max(1, Math.ceil((new Date(dates.end).getTime() - new Date(dates.start).getTime()) / (1000 * 3600 * 24)) + 1); };
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
      ...
      <main className="flex-1 relative h-full flex flex-col bg-gray-50">
        ...
        {/* --- GLOBAL HEADER --- */}
        <header className="absolute top-0 right-0 p-6 z-30 flex items-center gap-4">
           ...
           <div className="flex items-center gap-2">
              <div className="flex items-center bg-white rounded-lg border border-gray-200 shadow-sm p-1">
                 <button className="px-3 py-1.5 hover:bg-gray-50 rounded text-sm font-bold text-gray-700">₹</button>
                 <div className="w-px h-4 bg-gray-200"></div>
                 <button className="px-3 py-1.5 hover:bg-gray-50 rounded text-lg">🇺🇸</button>
                 <div className="w-px h-4 bg-gray-200"></div>
                 <button className="px-3 py-1.5 hover:bg-gray-50 rounded text-sm font-bold text-gray-700">°C</button>
              </div>
              <div className="relative">
                 <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="w-10 h-10 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center text-sm font-bold text-gray-700 hover:shadow-md transition-all relative">
                   {session.user.email?.[0].toUpperCase()}
                   <div className="absolute bottom-0 right-0 w-3 h-3 bg-white rounded-full flex items-center justify-center border border-gray-200"><span className="text-[8px]">⌄</span></div>
                 </button>
                 {showProfileMenu && (
                    <div className="absolute right-0 top-14 w-64 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden py-2 animate-fade-in-up origin-top-right">
                       <button onClick={()=>{setShowCreateModal(true); setShowProfileMenu(false);}} className="w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center gap-3 text-sm font-bold text-gray-700"><span>➕</span> New Trip</button>
                       ...
                       <button className="w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center gap-3 text-sm font-bold text-gray-700"><span>👑</span> Manage Subscription</button>
                       <div className="h-px bg-gray-100 my-1"></div>
                       <button className="w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center gap-3 text-sm font-bold text-gray-500"><span>ℹ️</span> About</button>
                       <div className="h-px bg-gray-100 my-1"></div>
          ...
          <div className="h-full overflow-y-auto">
             <div className="bg-black p-12 m-6 rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col justify-center min-h-[300px]">
                ...
                <div className="relative z-10 text-white">
                   ...
                   <button onClick={() => setShowCreateModal(true)} className="bg-white text-black px-8 py-4 rounded-2xl font-bold text-sm hover:scale-105 transition-transform inline-flex items-center gap-2 shadow-xl shadow-white/5"><span>✨</span> Plan a New Trip</button>
                </div>
             </div>
                ...
                <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
                   {['Paris', 'Tokyo', 'Bali', 'Dubai', 'New York'].map((city, i) => (
                      <div key={i} className="min-w-[200px] h-32 bg-white rounded-2xl border border-gray-200 p-4 flex flex-col justify-end hover:shadow-lg transition-all cursor-pointer group relative overflow-hidden">
                         ...
                         <div className="relative z-10 text-white"><span className="text-xs font-medium opacity-80">Explore</span><h4 className="font-bold text-lg">{city}</h4></div>
                      </div>
                   ))}
                </div>
             </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            {stepOptions.map((place) => (
                                <div key={place.id} className="relative group">
                                    ...
                                    {/* ✨ THE GENIUS BUTTON (Added to Card) */}
                                    <button 
                                      onClick={(e) => { e.stopPropagation(); openAiAssistant(place); }}
                                      className="absolute top-2 left-2 w-8 h-8 bg-gradient-to-tr from-purple-500 to-blue-500 rounded-full flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform z-10"
                                      title="Ask AI Genius"
                                    >
                                      ✨
                                    </button>
           ...
           <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
              <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden flex flex-col h-[500px]">
                 ...
                 <div className="bg-gradient-to-r from-purple-600 to-blue-600 p-6 flex justify-between items-start text-white">
                    <div>
                       <div className="text-[10px] uppercase font-bold tracking-widest opacity-80 mb-1">AI Concierge</div>
                       <h3 className="font-black text-xl leading-tight">{aiActivePlace.name}</h3>
                    </div>
                 ...
                 <div className="flex-1 bg-gray-50 p-4 overflow-y-auto space-y-3">
                    {aiChatHistory.map((msg) => (
                       <div key={msg.id} className={`flex ${msg.isMe ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[85%] p-3 rounded-2xl text-xs font-medium leading-relaxed ${msg.isMe ? 'bg-black text-white rounded-br-none' : 'bg-white border border-gray-200 text-gray-800 rounded-bl-none shadow-sm'}`}>
                             {msg.text}
                          </div>
                       </div>
                    ))}
                 ...
                 <div className="p-4 bg-white border-t border-gray-100">
                    ...
                    <div className="flex gap-2">
                       <input 
                         className="flex-1 bg-gray-100 rounded-xl px-4 py-3 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-purple-500/20" 
        ...
        {activeView === 'PLAN' && isLoaded && (
           <div className="h-full w-full relative">
              <GoogleMap mapContainerStyle={MAP_STYLES} center={mapCenter} zoom={12} options={{ disableDefaultUI: false, zoomControl: true }}>
                 ...
                 {/* SHOW REAL ROUTE OR STRAIGHT LINES */}
                 {directionsResponse ? (
              ...
              {/* FLOATING CONTROL TO CALCULATE ROUTE */}
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-white rounded-full shadow-2xl p-2 flex gap-2 animate-fade-in-up">
                 ...
                 <button onClick={() => setDirectionsResponse(null)} className="bg-white text-gray-500 border border-gray-200 px-4 py-3 rounded-full font-bold text-xs hover:bg-gray-50">
                    Reset
                 </button>
              </div>
           </div>
