    ...
    return (
        <section className="relative min-h-[95vh] flex flex-col overflow-hidden bg-[#F2F2F7] dark:bg-black">
            ...
            <nav className="absolute top-0 left-0 right-0 z-50 p-6">
                <div className="container mx-auto max-w-6xl flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        {user ? (
                            <Link href="/dashboard">
                                <button className="px-5 py-2.5 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border border-gray-200 dark:border-gray-700 rounded-full text-sm font-semibold text-gray-900 dark:text-white hover:bg-white dark:hover:bg-gray-800 transition-colors">
                                    Dashboard
                                </button>
                            </Link>
                        ) : (
                            <>
                                <Link href="/login">
                                    <button className="px-5 py-2.5 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">
                                        Log in
                                    </button>
                                </Link>
                                <Link href="/signup">
                                    <button className="px-5 py-2.5 bg-black dark:bg-white text-white dark:text-black rounded-full text-sm font-semibold hover:scale-105 transition-transform shadow-lg">
                                        Sign up
                                    </button>
                                </Link>
                            </>
            ...
            <div className="flex-1 flex items-center">
                <div className="container mx-auto px-4 relative z-10 max-w-6xl pt-20">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                        ...
                        >
                            ...
                            <div className="flex items-center justify-center lg:justify-start gap-6 pt-4">
                                <div className="flex -space-x-4 pl-4">
                                    {[1, 2, 3, 4].map(i => (
                                        <div key={i} className="w-12 h-12 rounded-full border-4 border-[#F2F2F7] dark:border-black bg-gray-200 overflow-hidden relative z-0 hover:z-10 transition-all hover:scale-110">
                                            <Image src={`https://i.pravatar.cc/100?img=${i + 10}`} width={48} height={48} alt="User" />
                                        </div>
                                    ))}
                                </div>
                        ...
                        >
                            <div className="relative z-10 flex flex-col gap-4">
                                ...
                                <motion.div
                                    layoutId="main-card"
                                    className="bg-white/70 dark:bg-gray-800/60 backdrop-blur-2xl border border-white/20 p-6 rounded-[2rem] shadow-2xl"
                                >
                                ...
                                {/* Floating Pills - Moved to normal flow */}
                                <div className="flex gap-3 overflow-x-auto pb-2 pt-2 no-scrollbar mask-linear">
