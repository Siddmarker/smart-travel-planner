...
// --- UPDATED INTERFACE: Props are now Optional (?) ---
interface SidebarProps {
  ...
  // Trip props are optional to support "Discover Mode"
  tripPlan?: any[]; 
...
export default function Sidebar({ 
  ...
  // Internal state for "Discover Mode" (if no props provided)
  ...
  // Only show PLANNER if we have the capability to add to a trip
  const showPlanner = !!onAddToTrip;
  ...
  useEffect(() => {
    async function fetchCities() {
      ...
      if (data) {
        ...
        if (!currentCity && uniqueCities.length > 0) setInternalCity(uniqueCities[0]);
      }
    }
    fetchCities();
  }, []);
  ...
  // Handle Internal Selection (if no prop provided)
  const handleCityClick = (city: string) => {
    if (onCitySelect) onCitySelect(city);
    else setInternalCity(city);
  };
  
  const handleFilterClick = (mode: string) => {
    ...
    <div className="w-96 h-screen bg-white shadow-xl z-20 flex flex-col border-r border-gray-200">
      <div className="p-5 border-b border-gray-100">
        ...
        <div className="flex gap-4 mt-4 text-xs font-bold border-b">
          <button onClick={() => setActiveTab('EXPLORE')} className={`pb-2 ${activeTab === 'EXPLORE' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400'}`}>
            DISCOVER
          </button>
          {showPlanner && (
            <button onClick={() => setActiveTab('PLANNER')} className={`pb-2 ${activeTab === 'PLANNER' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-400'}`}>
              AI PLANNER ✨
            </button>
          )}
        </div>
              ...
              <>
                <div className="p-4 bg-white border-b sticky top-0 z-10">
                   <div className="flex justify-between items-center mb-2">
                      <h2 className="text-lg font-black text-gray-800">{currentCity}</h2>
                   ...
                   <div className="flex justify-between items-center">
                     ...
                     <button onClick={onResetApp} className="text-[10px] text-red-500 font-bold underline hover:text-red-700">
                       Change Trip
                     </button>
                   </div>
                </div>
                ...
                <div className="space-y-3 p-2">
                  {filteredCityPlaces.map((place) => (
                    <div key={place.id} className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm flex flex-col gap-2 hover:border-blue-400 transition-all">
                      ...
                      {onAddToTrip && (
                        <button onClick={() => onAddToTrip({...place, slot: 'Saved'})} className="self-start text-[10px] font-bold text-white bg-black px-3 py-1.5 rounded-full hover:bg-gray-800">
                          + Add to List
                        </button>
                      )}
                    </div>
              ...
              <>
                <div className="p-3 bg-white mb-2 shadow-sm">
                  <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                    {FILTERS.map((mode) => (
                      <button key={mode} onClick={() => handleFilterClick(mode)} className={`px-3 py-1.5 rounded-full text-[10px] font-bold border whitespace-nowrap ${activeFilter === mode ? 'bg-black text-white' : 'bg-white text-gray-500'}`}>{mode}</button>
                    ))}
                  </div>
                </div>
          ...
          <div className="p-4 h-full flex flex-col">
            {stage === 'IDLE' && (
              <div className="text-center mt-10">
                ...
                <p className="text-xs text-gray-500 mb-6 px-4">I will curate a complete <b>{totalDays}-Day</b> journey for you.</p>
            ...
            {/* ... Wizard stages identical to before ... */}
